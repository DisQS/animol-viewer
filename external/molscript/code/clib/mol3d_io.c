...
#ifdef CUSTOM_ATOF
extern float CUSTOM_ATOF(const char*);

inline float atof_(const char *s)
{
  return CUSTOM_ATOF(s);
}
#else
inline float atof_(const char *s)
{
...
#define PDB_RECORD_LENGTH 80
  ...
  while (str) {			/* atom records */
	...
	(str_eq_min (HETATM, record))) {
      ...
      new_at->bfactor = atof_ (record + 60);
      record[60] = '\0';
      new_at->occupancy = atof_ (record + 54);
      record[54] = '\0';
      new_at->xyz.z = atof_ (record + 46);
      record[46] = '\0';
      new_at->xyz.y = atof_ (record + 38);
      record[38] = '\0';
      new_at->xyz.x = atof_ (record + 30);
