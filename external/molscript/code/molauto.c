...
/*============================================================*/
const char molauto_program_str[] = "MolAuto v1.1.1";
const char molauto_copyright_str[] = "Copyright (C) 1997-1998 Per J. Kraulis";
...
const char *molauto_output_filename = NULL;
FILE *molauto_outfile = NULL;
...
output_hsb_decrement (void)
{
  if (colour_mode && !nice_mode) {
    fprintf (molauto_outfile, "  set planecolour hsb %.4g 1 1;\n", hue);
    hue -= decrement;
    if (hue < 0.0) hue = 0.0;
  }
}
...
/*------------------------------------------------------------*/
char *
molauto_process_arguments (void)
{
  int slot;
  const char *str;
  ...
  if ((args_number <= 1) || (args_exists ("-h"))) {
    ...
    fprintf (stderr, "  -out file  output to the named file, instead of stdout\n");
    fprintf (stderr, "  -h         output this message\n");
    fprintf (stderr, "----- %s, %s\n", molauto_program_str, molauto_copyright_str);
  ...
  if (slot) {
    ...
    if (str) {
      args_flag (slot + 1);
      molauto_output_filename = str;
    } else {
      fatal_error("no filename given for option -out");
    }
  }
...
output_secondary_structure (mol3d *mol)
{
  ...
  if (colour_mode) {
    ...
    if (nice_mode) {
      fprintf (molauto_outfile, "  set colourparts on, residuecolour amino-acids rainbow;\n\n");
    } else {
      int parts = 0;
  ...
  for (res = mol->first; res; res = res->next) {
	...
	fprintf (molauto_outfile, "  coil from %s to ", first->name);
	switch (toupper (res->secstruc)) {
	case '-':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	  first = NULL;
	  break;
	case 'T':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	...
	break;
      ...
      case 'T':
	output_hsb_decrement();
	fprintf (molauto_outfile, "  turn from %s to ", first->name);
	switch (toupper (res->secstruc)) {
	case '-':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	  first = NULL;
	...
	output_hsb_decrement();
	if (cylinder_mode) {
	  fprintf (molauto_outfile, "  cylinder from %s to ", first->name);
	} else {
	  fprintf (molauto_outfile, "  helix from %s to ", first->name);
	}
	switch (toupper (res->secstruc)) {
	case '-':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	...
	case 'E':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	  fprintf (molauto_outfile, "  turn from %s to %s;\n", prev->name, res->name);
	  first = res;
	  break;
	}
	break;
      ...
      case 'E':
	output_hsb_decrement();
	fprintf (molauto_outfile, "  strand from %s to ", first->name);
	switch (toupper (res->secstruc)) {
	case '-':
	  fprintf (molauto_outfile, "%s;\n", prev->name);
	  first = NULL;
...
output_ligand (mol3d *mol)
{
  ...
  for (res = mol->first; res; res = res->next) {
	...
	fprintf (molauto_outfile, "  cpk in residue %s;\n", res->name);
      } else {
	fprintf (molauto_outfile, "  cpk in require residue %s and type %s;\n",
		res->name, res->type);
      }
    } else if (res3d_unique_name (mol, res)) {
      fprintf (molauto_outfile, "  %s in residue %s;\n", render_type, res->name);
    } else {
      fprintf (molauto_outfile, "  %s in require residue %s and type %s;\n", render_type,
	      res->name, res->type);
    }
  }
}
...
output_quoted_string (char *str)
{
  ...
  putc ('"', molauto_outfile);
  for ( ; *str; str++) {
    if (*str == '"') putc ('\\', molauto_outfile);
    putc (*str, molauto_outfile);
  }
  putc ('"', molauto_outfile);
}
...
/*------------------------------------------------------------*/
int molauto (int argc, char *argv[])
{
  char *pdbfilename;
  mol3d *mol;

  args_initialize (argc, argv);
  pdbfilename = molauto_process_arguments();

  if (molauto_output_filename)
  {
    molauto_outfile = fopen(molauto_output_filename, "w");

    if (molauto_outfile == NULL)
      fatal_error("Could not create output file");
  }
  else
    molauto_outfile = stdout;
  ...
  fprintf (molauto_outfile, "! MolScript v2.1 input file\n");
  fprintf (molauto_outfile, "! generated by %s\n\n", molauto_program_str);
  ...
  fprintf (molauto_outfile, "plot\n\n  read mol ");
  output_quoted_string (pdbfilename);
  fprintf (molauto_outfile, ";\n\n");

  if (centre_mode)
    fprintf (molauto_outfile, "  transform atom * by centre position atom *;\n\n");

  if (!nice_mode) fprintf (molauto_outfile, "  set segments 8;\n\n");

  if (thin_mode)
    fprintf (molauto_outfile, "  set strandthickness 0, helixthickness 0, coilradius 0;\n\n");
  ...
  if (molauto_output_filename)
    fclose(molauto_outfile);

  mol3d_delete_all (mol);
