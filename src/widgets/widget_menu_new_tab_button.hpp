...
class widget_menu_new_tab_button : public plate::ui_event_destination
{
  ...
  void create_button() noexcept
  {
    ...
    const auto bitmap = [&]
    {
      ...
      for (int row = 0; row < sz; ++row)
        for (int col = 0; col < sz; ++col)
        {
          //if (  ((row < sz*3/8 || row > sz*5/8) && (col < sz*1/10 || col > sz*9/10))
          //    ||((col < sz*3/8 || col > sz*5/8) && (row < sz*1/10 || row > sz*9/10)) )
          if (col < sz*1/10 || row > sz*9/10 || (col > sz*9/10 && row > sz/2) || (row < sz*1/10 && col < sz/2))
          {
            int offset = (row * w) + (col * 4);
            bitmap[offset+3] = 255;
          }
    ...
    {
      //TODO: make this more general (ask js for url???)
