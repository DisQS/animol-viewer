...
class widget_menu_save_button : public plate::ui_event_destination
{
...
protected:
  ...
  void create_menu_list() noexcept
  {
    ...
    list.push_back("Upload current style"sv);

    widget_menu_list_->set_text(std::move(list));

    widget_menu_list_->set_selection_cb([this] (std::uint32_t i, std::string_view s)
    {
      if (s == "Upload current style")
      {
        emscripten_run_script(std::string{"addStyleToUpload('" + viewer_->ui_->name_ + "')"}.data());
